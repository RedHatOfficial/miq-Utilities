...
def send_service_provision_update_email(request, to, from, update_message, cfme_hostname)
  ...
  $evm.log(:info, "service_task => #{service_task}") if @DEBUG
...
begin  
  ...
  request ||= $evm.root['service_template_provision_task'].miq_request unless $evm.root['service_template_provision_task'].nil?
