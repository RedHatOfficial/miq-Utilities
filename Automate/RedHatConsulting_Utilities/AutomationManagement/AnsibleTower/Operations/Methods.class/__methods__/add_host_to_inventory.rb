...
begin  
  ...
  $evm.log(:info, "Starting Ansible Tower REST API call to add a host to the inventory")
  ...
  error('No IP addresses associated with VM') if vm.ipaddresses.empty?
  vm_ip_address = vm.ipaddresses.first
  $evm.log(:info, "Host IP address to be added to Tower Inventory: #{vm_ip_address}") if @DEBUG
  ...
  $evm.log(:info, "Host already present in Tower inventory: Host ID = #{host_id}") if ( @DEBUG and host_present_in_inventory )
  $evm.log(:info, "Host not yet present in Tower inventory") if ( @DEBUG and !host_present_in_inventory )
  ...
  $evm.log(:info, "VM #{vm_hostname} with IP address #{vm_ip_address} successfully added to Ansible Tower inventory [ #{inventory_name} ]")
  exit MIQ_OK

rescue => err
  $evm.log(:error, "[(#{err.class})#{err}]\n#{err.backtrace.join("\n")}")
  error("Error adding host to Ansible Inventory: #{err}")
end
