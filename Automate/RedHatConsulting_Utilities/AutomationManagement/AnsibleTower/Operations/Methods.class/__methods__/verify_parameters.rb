...
begin
  ...
  job_template      = get_param(:job_template)      || options[:job_template]
  job_template_name = get_param(:job_template_name) || options[:job_template_name] || options[:dialog_job_template_name]
  job_template_id   = get_param(:job_template_id)   || options[:job_template_id]   || options[:dialog_job_template_id]
  
  if job_template.blank? && job_template_name.blank? && job_template_id.blank?
    error("Did not find one of <job_template, job_template_name, job_template_id>. Skip running Job")
  else
    $evm.object['job_template']      = job_template      if !job_template.blank?
    $evm.object['job_template_name'] = job_template_name if !job_template_name.blank?
    $evm.object['job_template_id']   = job_template_id   if !job_template_id.blank?
    
    # NOTE: note sure why have to do this
    ansible_params = options.select { |k,v| k.to_s =~ /dialog_param|param_/ }
    ansible_params.each { |k,v| $evm.object[k.to_s] = v; $evm.root[k.to_s] = v }
  end
end
