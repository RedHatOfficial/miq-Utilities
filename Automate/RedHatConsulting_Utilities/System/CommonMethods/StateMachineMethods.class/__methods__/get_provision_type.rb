# Determine the provision type to use for the current miq_provision in the VMProvision_*/* state machine
#
...
begin
  ...
  custom_provision_type = prov.get_option(:custom_provision_type) || (!prov.get_option(:ws_values).blank? && prov.get_option(:ws_values)[:custom_provision_type])
