...
begin
  ...
  if vm.ipaddresses.nil? || vm.ipaddresses.empty?
    $evm.log(:info, "VM IP addresses not detected yet, perform VM refresh and retry") if @DEBUG
    vm.refresh
    automate_retry(30, 'Wait for VM refresh to detect VM IP addresses')  
  else
    $evm.log(:info, "vm.ipaddresses => #{vm.ipaddresses}") if @DEBUG
